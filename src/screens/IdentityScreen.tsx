...
import { DataRetentionSettingsSheet } from '../components/DataRetentionSettingsSheet';
...
export default function IdentityScreen() {
  ...
  const [showDataRetentionSettings, setShowDataRetentionSettings] = useState(false);
  ...
  return (
    <div className="w-full h-full overflow-y-auto no-scrollbar bg-skrim-bg relative pb-20">
            ...
            <button
              onClick={() => {
                ...
                if (archivedSparks.length === 0) {
                  ...
                  setToastMessage("Select a day to create a Highlight!");
           ...
           <>
             ...
             >
               ...
               <div className="p-4 flex flex-col gap-2 overflow-y-auto pb-8">
                 ...
                 <button className="flex items-center gap-4 p-4 hover:bg-white/5 rounded-2xl transition" onClick={() => {
                   setShowSettingsMenu(false);
                   setShowDataRetentionSettings(true);
                 }}>
                   <div className="w-10 h-10 rounded-full bg-[#00F0FF]/20 flex items-center justify-center shrink-0">
                     <Archive className="w-5 h-5 text-[#00F0FF]" />
                   </div>
                   <div className="flex-1 text-left">
                     <p className="font-bold text-white text-lg">Data Retention</p>
                     <p className="text-white/50 text-sm">Auto-delete posts, saves, messages & more</p>
                   </div>
                 </button>
      ...
      {/* Data Retention Settings Sheet */}
      <AnimatePresence>
        {showDataRetentionSettings && (
          <DataRetentionSettingsSheet onClose={() => setShowDataRetentionSettings(false)} />
        )}
      </AnimatePresence>
...
// One tile per day of past Sparks — tapping a day drops you into
// SparkViewer (in highlight mode, so expiry never blocks viewing)
// showing just that day's Sparks in order, ready to save as a Highlight.
      ...
      >
        ...
        <div className="px-6 flex justify-between items-center pb-4 shrink-0 border-b border-white/5">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <Plus className="w-5 h-5 text-[#B026FF]" /> Create Highlight
          </h2>
        ...
        <div className="p-4 overflow-y-auto pb-8">
          {archivedByDay.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-2">
              ...
              <p className="text-white/40 text-sm text-center max-w-[220px]">Post some Sparks first, then come back here to turn them into a Highlight.</p>
            </div>
          ) : (
